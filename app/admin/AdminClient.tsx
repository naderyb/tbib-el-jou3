...
export default function AdminClient() {
  ...
  const [activeTab, setActiveTab] = useState<"orders" | "restaurants">(
    "orders"
  );
  ...
  const ensureAdmin = async () => {
    try {
      ...
      const dbg = await fetch("/api/auth/admin-login", {
        credentials: "include",
      });
      if (dbg.ok) {
  ...
  const mapRowToOrder = (r: ApiOrderRow): Order => ({
    ...
    restaurant: {
      name: r.restaurant_name || (r.restaurant && r.restaurant.name),
    },
    status: r.status,
  ...
  const handleRealtime = useCallback((msg: any) => {
    ...
    if (
      ...
      setOrders((prev) =>
        prev.map((o) =>
          o.id === String(payload.order.id) ? mapRowToOrder(payload.order) : o
        )
      );
    } else if (
      (type === "order:new" || type === "order:created") &&
      payload?.order
    ) {
  ...
  const openOrderDetails = async (order: Order) => {
    ...
    try {
      ...
      const data = await res.json();
      setOrderDetails(data); // expects { ...order, items: [...] } or similar
    } catch (err) {
  ...
  const saveRestaurant = async () => {
    try {
      ...
      const payload: any = {
        ...
        delivery_fee:
          restaurantForm.delivery_fee !== "" &&
          restaurantForm.delivery_fee != null
            ? Number(restaurantForm.delivery_fee)
            : null,
        minimum_order:
          restaurantForm.minimum_order !== "" &&
          restaurantForm.minimum_order != null
            ? Number(restaurantForm.minimum_order)
            : null,
        delivery_time_min:
          restaurantForm.delivery_time_min !== "" &&
          restaurantForm.delivery_time_min != null
            ? Number(restaurantForm.delivery_time_min)
            : null,
        delivery_time_max:
          restaurantForm.delivery_time_max !== "" &&
          restaurantForm.delivery_time_max != null
        ...
        owner_id: restaurantForm.owner_id
          ? Number(restaurantForm.owner_id)
          : null,
        is_active:
          typeof restaurantForm.is_active === "boolean"
            ? restaurantForm.is_active
            : !!restaurantForm.is_active,
      };
  ...
  const fetchMenuForRestaurant = async (restaurantId: number) => {
    ...
    try {
      ...
      const res = await fetch(`/api/menu_items?restaurant_id=${restaurantId}`, {
        credentials: "include",
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
      ...
      setMenuItems(
        Array.isArray(data)
          ? data.map((mi: any) => ({ ...mi, price: Number(mi.price || 0) }))
          : []
      );
  ...
  const saveMenuItem = async () => {
    try {
      ...
      if (!ok) {
        alert(
          "Unauthorized: admin access required to create/update menu items."
        );
        return;
      }
      ...
      const payload: any = {
        ...
        // category is text
        category:
          menuForm.category && String(menuForm.category).trim() !== ""
            ? String(menuForm.category)
            : null,
        name: menuForm.name || null,
        description: menuForm.description || null,
        price:
          menuForm.price !== undefined && menuForm.price !== ""
        ...
        calories:
          menuForm.calories !== undefined && menuForm.calories !== ""
            ? Number(menuForm.calories)
            : null,
        preparation_time:
          menuForm.preparation_time !== undefined &&
          menuForm.preparation_time !== ""
            ? Number(menuForm.preparation_time)
            : null,
        ingredients: menuForm.ingredients || null,
        allergens: menuForm.allergens || null,
        sort_order:
          menuForm.sort_order !== undefined && menuForm.sort_order !== ""
            ? Number(menuForm.sort_order)
            : 0,
      };
      ...
      if (editingMenuItem) {
        ...
        if (!res.ok) {
          const err = await res.json().catch(() => ({}));
          throw new Error(
            err?.error || err?.message || "Failed to update menu item"
          );
        }
      } else {
        ...
        if (!res.ok) {
          const err = await res.json().catch(() => ({}));
          throw new Error(
            err?.error || err?.message || "Failed to create menu item"
          );
        }
      }
      ...
      if (selectedRestaurantId)
        await fetchMenuForRestaurant(Number(selectedRestaurantId));
  ...
  const deleteMenuItem = async (id: number) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to delete");
      if (selectedRestaurantId)
        await fetchMenuForRestaurant(selectedRestaurantId);
    } catch (err) {
        ...
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <div className="text-sm text-gray-700 hidden sm:block">
              {adminName ? `Hi, ${adminName}` : "Admin"}
            </div>
            <button
              onClick={() => setShowLogoutModal(true)}
              className="flex items-center space-x-2 px-3 py-1 border rounded hover:bg-gray-100"
            >
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      <th className="text-left py-4 px-6 font-semibold text-gray-900 font-poppins">
                        Order
                      </th>
                      <th className="text-left py-4 px-6 font-semibold text-gray-900 font-poppins">
                        Customer
                          ...
                          <td className="py-4 px-6">
                            <div className="text-sm">
                              <div className="text-gray-900 font-poppins">
                                Fee: {formatCurrency(r.delivery_fee)}
                              </div>
              ...
              <div className="glass-card p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition-colors font-poppins"
                  >
                    Add Menu Item
                  </button>
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b border-gray-200">
                      <tr>
                        <th className="text-left py-3 px-4 font-semibold text-gray-900 font-poppins">
                          Item
                        </th>
                        <th className="text-left py-3 px-4 font-semibold text-gray-900 font-poppins">
                          Price
                          ...
                          >
                            <td className="py-3 px-4">
                              <div>
                                ...
                                <div className="text-sm text-gray-500 font-poppins line-clamp-1">
                                  {mi.description}
                                </div>
                              </div>
                            </td>
                            <td className="py-3 px-4 font-semibold text-gray-900 font-poppins">
                            ...
                            <td className="py-3 px-4">
                              <span
                                className={`px-2 py-1 rounded-full text-xs font-medium font-poppins ${
                                  mi.is_available
                                    ? "bg-green-100 text-green-800"
                                    : "bg-gray-100 text-gray-800"
                                }`}
                  ...
                  <>
                    ...
                    <div className="grid md:grid-cols-2 gap-4 text-sm">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2 text-gray-500 text-xs">
                          ...
                          <span>
                            {orderDetails?.created_at || selectedOrder.createdAt
                              ? new Date(
                    ...
                    <div className="border-t pt-4">
                      ...
                      <div className="space-y-2">
                        {(
                          ...
                          (orderDetails.items || orderDetails.order?.items).map(
                            (it: any, idx: number) => (
                              <div
                                key={it.id ?? `${it.menu_item_id}-${idx}`}
                                className="flex justify-between items-center text-xs border-b last:border-0 pb-2"
                              >
                                <div>
                    ...
                    <div className="border-t pt-4 text-sm space-y-1">
                      ...
                      <div className="flex justify-between pt-1 border-t mt-2">
                        ...
                        <span className="font-semibold">
                          {formatCurrency(
                            orderDetails?.total ?? selectedOrder.total ?? 0
                          )}
                        </span>
                      </div>
                    </div>
          ...
          >
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <input
                  placeholder="Delivery time (label)"
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 items-center mb-2">
                  ...
                  <label className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setMenuForm({ ...menuForm, is_vegan: e.target.checked })
                      }
                    />{" "}
                    Vegan
                  </label>
                  <label className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      checked={!!menuForm.is_gluten_free}
                      onChange={(e) =>
                        setMenuForm({
                          ...menuForm,
